...
export default class SimpleShortcode {
    config: ConfigType
    data: {
        shortcode: string
        options: string
    ...
    render(){
        ...
        const $options = this.config.allowed_shortcodes.map((shortcode: string) => {
            const $option = document.createElement('option')
            $option.value = shortcode
            $option.innerHTML = shortcode
            if ( this.data.shortcode === shortcode ) {
        ...
        $input.placeholder = 'Shortcode-Optionen'
        if ( !!this.data.options && this.data.options !== undefined ) {
            $input.value = this.data.options
        }
        ...
        $select.addEventListener('change', async () => {
            $select.classList.toggle('changed')
        })
